...
//        selTraintype.selectByValue("trainType"); //ALL
   ...
   // @Test(enabled = false)
    ...
    public void testSelectContacts()throws Exception{
    	CheckAlert.occurrs(driver); // Alert Trip not found é lançado quando é o botão "Booking - linha 124" é clicado. 
        ...
        if (contactsList.size() == 1){
            ...
            contactsList.get(0).findElement(By.xpath("td[3]/input")).sendKeys(contactName);
            ...
            contactsList.get(0).findElement(By.xpath("td[5]/input")).sendKeys(idNumber);
            contactsList.get(0).findElement(By.xpath("td[6]/input")).sendKeys(phoneNumber);
        ...
        if (contactsList.size() > 1) {
            ...
            contactsList.get(i).findElement(By.xpath("td[7]/label/input")).click(); // td[] indexada a partir de (1) - correct
        }
    ...
    public void testTicketConfirm ()throws Exception{
        ...
        new WebDriverWait(driver, 60) .until(ExpectedConditions.alertIsPresent()); // Esperar max de (60s) até o Alert de confirmação do ticker ser lançado.       
    ...
    public void testTicketCollect ()throws Exception {
        ...
        /* Não existe o form de preserve_collect_order_status nem outro semelhante, 
         o form não foi implementado no html logo não é possível executar essa verficação */
